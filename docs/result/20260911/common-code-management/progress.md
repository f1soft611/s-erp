...
- 화면 로딩/재조회는 실제 백엔드 API `/api/v1/co/master/common-code/...`를 호출하도록 연결했다.
- 저장 처리도 신규/수정/삭제 변경 사항을 실제 API로 전송하도록 연결했다.
- 페이지의 조회 버튼은 dirty 상태가 없을 때 실제 데이터 재조회까지 실행한다.
- MyBatis XML의 `tb_common_code_group`, `tb_common_code_item` 스키마와 nullable FK 바인딩(`jdbcType=BIGINT`)을 계약 테스트로 검증한다.
- 최종 검증으로 프론트엔드 서비스 테스트와 백엔드 컨트롤러/매퍼 계약 테스트를 실행했다.
