...
- 미사용 `FeedList` 컴포넌트와 테스트 의존 제거
- 공통 보기 컴포넌트의 도메인 전용 접근성 라벨을 props 계약으로 변경
- 초기·재조회·정상 상태에서 동일한 `ContentSplitLayout`과 `PinnedViewToolbar` 조립 사용
...
- `git diff --check`
  - 공백 오류 없음
- 리팩터링 핵심 테스트
  - 공통·피드·공지 모드 테스트 10개 통과, 실패 0개
- `FeedList` 참조 검색
  - `frontend/src`에서 참조 없음
- 브라우저
  - 공지사항 화면 진입 확인
