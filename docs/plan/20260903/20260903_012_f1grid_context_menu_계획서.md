...
F1-Grid는 헤더 메뉴(정렬/필터/고정/숨김)만 제공하고, 바디 영역 우클릭 컨텍스트 메뉴는 `docs/guide/F1-GRID.md` 25번 항목에 "미구현"으로만 문서화되어 있다. 메뉴관리 화면(F1Tree 기반)은 그리드 바깥의 툴바 버튼(`루트 메뉴 추가`, `하위 메뉴 추가`)으로 행 추가를 처리하고 있어 그리드 내부 조작과 화면 툴바 조작이 이원화되어 있다.
...
1. `docs/guide/F1-GRID.md` 25번 "Grid Context Menu" 섹션을 실제 구현 메뉴 구성/동작으로 갱신하고 "⚠️ 아직 미구현" 표기를 제거한다.
